<div class="intro-y flex flex-col sm:flex-row items-center mt-8">
    ...
    <div class="w-full sm:w-auto flex mt-4 sm:mt-0">
        <a href="<?php echo base_url("admin/{$viewFolder}/add");?>" class="button text-white bg-theme-1 shadow-md mr-2">Yeni Kayıt Ekle</a>
    ...
    <table class="table table-report table-report--bordered display datatable w-full">
        <thead>
            <tr>
                ...
                <th class="whitespace-no-wrap w-16">İkon</th>
                <th class="whitespace-no-wrap">İçerik Adı</th>
                <th class="text-center whitespace-no-wrap">Tür</th>
        ...
        <tbody class="sortable" data-url="<?=base_url("admin/orderUpdate/{$modelName}")?>">
            ...
            <tr class="intro-x" id="ord-<?=$rows->id?>">
                ...
                <td><i class="fa fa-2x fa-<?=$rows->icon?>"></i></td>
                <td>
                    <a href="<?=base_url("admin/{$viewFolder}/edit/$rows->id")?>" class="font-medium"><?=$rows->title?></a> 
                    <div class="text-gray-600 text-xs"><?=$rows->link?></div>
                </td>
